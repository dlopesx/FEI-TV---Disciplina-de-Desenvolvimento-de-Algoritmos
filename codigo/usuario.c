...
void cadastrar_usuario(){ 
    ...
    Usuario usuario = {0}; // struct base pra cadastrar um por vez na fun�ao
    ...
    while (1){
        ...
        // validando pra ver se o username digitado � v�lido 
        if (username_invalido == true){
            printf("Esse username j� est� cadastrado. Digite um v�lido: ");
            fgets(usuario.username, sizeof(usuario.username), stdin);
            usuario.username[strlen(usuario.username) - 1] = '\0';
        }
        else{
    ...
    printf("\nUsu�rio cadastrado com sucesso! Voltando ao menu de login...\n");
    Sleep(2000);
    system("cls");
}
...
void logar_usuario(char *usernamecpy){
    ...
    while(1){
        ...
        if (verificar_username(nome_usuario, username, senha) == 1){
            ...
            printf("Esse username n�o existe. Tente novamente ou 0 para voltar: ");
            fgets(username, sizeof(username), stdin);
            username[strcspn(username, "\n")] = '\0';
            if (strcmp(username, "0") == 0){
               printf("Voltando...");
